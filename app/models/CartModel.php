...
class CartModel
{
    ...
    public function getCartItemsByUser(int $userId): array
    {
        ...
        try {
            ...
            LEFT JOIN discounts d ON ci.discount_code = d.discount_code AND ci.product_id = d.product_id
    ...
    {
        try {
            $stmt = $this->conn->prepare("SELECT * FROM discounts 
    ...
    {
        try {
            $stmt = $this->conn->prepare("UPDATE discounts 
    ...
    public function addOrUpdateCartItem(int $cartId, int $productId, int $quantity, float $price, ?string $discountCode = null): bool
    {
        try {
            ...
            if ($existingQty > 0) {
                ...
                $stmt = $this->conn->prepare("INSERT INTO {$this->cartItemsTable} 
                    (cart_id, product_id, quantity, price, discount_code, created_at)
                    VALUES (:cart_id, :product_id, :quantity, :price, :discount_code, NOW())");
            }
