...
/**
 * Lớp Utils chứa các hàm tiện ích dùng chung trong ứng dụng.
 */
class Utils
{
    ...
    private static function optimizeAndSaveImage(string $sourcePath, string $destPath, string $mimeType): bool
    {
        ...
        $image = null; $success = false;
        try {
            switch ($mimeType) {
                case 'image/jpeg':
                    if (!(imagetypes() & IMG_JPG)) { throw new \RuntimeException('JPEG support is not enabled in GD.'); }
                    ...
                    break;
                case 'image/png':
                    if (!(imagetypes() & IMG_PNG)) { throw new \RuntimeException('PNG support is not enabled in GD.'); }
                    ...
                    break;
                case 'image/webp':
                    if (!(imagetypes() & IMG_WEBP)) { throw new \RuntimeException('WebP support is not enabled in GD.'); }
            ...
            if (!$success) { error_log("Utils Error: Failed to save optimized image to '$destPath' for type '$mimeType'."); }
            return $success;
        } catch (\Throwable $e) {
    ...
    public static function uploadImage(array $file, string $filePrefix, ?string $customName = null): array
    {
        ...
        if (!isset($file['error']) || is_array($file['error'])) { return ['success' => false, 'message' => 'Thông số upload không hợp lệ.']; }
        switch ($file['error']) {
            case UPLOAD_ERR_OK: break;
            case UPLOAD_ERR_NO_FILE: return ['success' => false, 'message' => 'Không có file nào được upload.'];
            case UPLOAD_ERR_INI_SIZE: case UPLOAD_ERR_FORM_SIZE: return ['success' => false, 'message' => 'Kích thước ảnh vượt quá giới hạn cho phép.'];
            default: return ['success' => false, 'message' => 'Lỗi không xác định khi upload file.'];
        }

        // Step 2: Validate uploaded file
        $tempPath = $file['tmp_name'];
        if (!is_uploaded_file($tempPath)) { error_log("Utils Security Alert: Possible file upload attack. Path: " . $tempPath); return ['success' => false, 'message' => 'File upload không hợp lệ.']; }

        // Step 3: Check MIME type and size
        $finfo = finfo_open(FILEINFO_MIME_TYPE); $mimeType = finfo_file($finfo, $tempPath); finfo_close($finfo);
        if (!in_array($mimeType, self::ALLOWED_IMAGE_TYPES, true)) { return ['success' => false, 'message' => 'Chỉ chấp nhận ảnh JPEG, PNG hoặc WEBP. Loại file phát hiện: ' . $mimeType]; }
        if (filesize($tempPath) > self::MAX_UPLOAD_SIZE_MB * 1024 * 1024) { return ['success' => false, 'message' => 'Ảnh quá lớn (tối đa ' . self::MAX_UPLOAD_SIZE_MB . 'MB)']; }

        // Step 4: Generate filename and path
        $extensions = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/webp' => 'webp'];
        $ext = $extensions[$mimeType] ?? pathinfo($file['name'], PATHINFO_EXTENSION);
        if ($customName !== null) { $customName = preg_replace('/[^a-zA-Z0-9_-]/', '_', $customName); $customName = trim($customName, '_-'); $customName = mb_substr($customName, 0, 100); }
        ...
        if (!is_dir($absoluteDir)) {
            if (!mkdir($absoluteDir, self::UPLOAD_DIR_PERMISSIONS, true) && !is_dir($absoluteDir)) { error_log("Utils Error: Failed to create upload directory: " . $absoluteDir); return ['success' => false, 'message' => 'Không thể tạo thư mục lưu ảnh.']; }
        }
        if (!is_writable($absoluteDir)) { error_log("Utils Error: Upload directory is not writable: " . $absoluteDir); return ['success' => false, 'message' => 'Không có quyền ghi vào thư mục lưu ảnh.']; }
        $finalPath = $absoluteDir . '/' . $fileName;

        // Step 6: Optimize and save
        if (!self::optimizeAndSaveImage($tempPath, $finalPath, $mimeType)) { return ['success' => false, 'message' => 'Không thể xử lý hoặc lưu ảnh đã tối ưu.']; }
    ...
    {
        if (defined('APP_BASE_URL')) { return rtrim(APP_BASE_URL, '/') . '/' . ltrim($relativePath, '/'); }
    ...
    {
        $errors = [];
        if ($data === null) { return ['_error' => "Dữ liệu đầu vào không hợp lệ (null)."]; }
    ...
    {
        if (headers_sent($file, $line)) { error_log("Utils Error: Headers already sent in $file on line $line."); echo '{"success": false, "message": "Server configuration error."}'; die(); }
        http_response_code($status);
        header('Content-Type: application/json; charset=utf-8');
        $jsonData = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        if ($jsonData === false) { $jsonError = json_last_error_msg(); error_log("Utils Error: Failed to encode JSON. Error: " . $jsonError); http_response_code(500); echo '{"success": false, "message": "Server error processing response data.", "json_error": "' . $jsonError . '"}';
        } else { echo $jsonData; }
        die();
    }
